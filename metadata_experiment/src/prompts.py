...
_RETRY_SUFFIX = (
    ...
    " topics 必须从受控词表中选 0–2 个，keywords 恰好 3 个，characters 最多 4 个。"
)
...
def build_metadata_prompt(chunk_text: str, topics: list[TopicDefinition], *, retry: bool = False) -> str:
    ...
    prompt = (
        ...
        "3. 若片段包含两个对理解事件都重要的主题，可保留两个 topics；不得仅因出现家庭成员就选 family；"
        "不得为填充数量添加弱相关 topic。\n"
        "4. keywords 必须恰好 3 个；characters 最多 4 个。\n"
        "5. 验血/抽血/生产 → medical；枪毙/土改/食堂 → politics；赌局/输家产 → gambling；"
        "买牛/耕田 → labor；藏米/缺粮 → livelihood。\n\n"
        f"可选 topic：\n{topic_lines}\n\n"
