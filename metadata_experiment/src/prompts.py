...
def build_metadata_prompt(chunk_text: str, topics: list[TopicDefinition]) -> str:
    ...
    return (
        ...
        "4. topic_evidence 必须与 topics 一一对应：数量相同、顺序相同；"
        "topics[i] 的证据写在 topic_evidence[i]。若 topics=[]，topic_evidence 也必须为 []。"
        "证据应引用或概括当前片段中的具体事件，不得写全书背景判断。"
        "topic_evidence 仅用于 Prompt 调试，不会入库，程序也不会校验其内容。\n"
        ...
        "14. importance 必须是 1、2、3、4、5 中的一个整数。\n\n"
        f"可选 topic 列表：\n{topic_lines}\n\n"
        f"文本片段：\n{chunk_text}\n"
    )
