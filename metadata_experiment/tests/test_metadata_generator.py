...
def test_metadata_generator_preserves_llm_topic_order():
    payload = {
        "characters": ["家珍"],
        "topics": ["family", "medical"],
        "keywords": ["一", "二", "三"],
    }
    metadata, invalid = normalize_metadata_payload(payload, {"family", "medical"})
    assert invalid == []
    assert metadata is not None
    assert metadata.topics == ["family", "medical"]
